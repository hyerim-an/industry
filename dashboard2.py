...
with col_result_left:
    ...
    # 품질 등급에 따른 시각적 경고 (정의된 등급에 따라 아이콘 선택)
    if predicted_defect == 3:
        st.error(f"🚨 **불량(Bad)** - 품질 등급: {predicted_defect}")
        st.error("⚠️ **설비 점검 필요** - 공정 매개변수를 즉시 검토하고 설비를 점검하세요.")
...
with col_result_right:
    ...
    # hue 없이 palette을 단일 색상으로 지정하되, 최적화된 barplot 사용
    sns.barplot(
...
with st.expander("📋 맞춤형 공정 인사이트 (핵심 3가지)", expanded=True):
    ...
    if abs(force_deviation) < 1:
        st.write(f"   • ✅ 현재 {weld_force:.2f} bar는 최적 범위 내 (평균 {force_mean:.2f}±{force_std:.2f} bar)")
        st.write(f"   • 권고: 현재 설정 유지")
    elif force_deviation > 1:
        st.write(f"   • ⚠️ 현재 {weld_force:.2f} bar는 평균보다 높음 (평균 {force_mean:.2f} bar)")
        st.write(f"   • 권고: {force_mean:.2f}±{force_std:.2f} bar 범위로 감소 조정")
    else:
        st.write(f"   • ⚠️ 현재 {weld_force:.2f} bar는 평균보다 낮음 (평균 {force_mean:.2f} bar)")
    ...
    if abs(current_deviation) < 1:
        st.write(f"   • ✅ 현재 {weld_current:.2f} kA는 최적 범위 내 (평균 {current_mean:.2f}±{current_std:.2f} kA)")
        st.write(f"   • 권고: 현재 설정 유지")
    elif current_deviation > 1:
        st.write(f"   • ⚠️ 현재 {weld_current:.2f} kA는 평균보다 높음 (평균 {current_mean:.2f} kA)")
        st.write(f"   • 권고: {current_mean:.2f}±{current_std:.2f} kA 범위로 감소 조정")
    else:
        st.write(f"   • ⚠️ 현재 {weld_current:.2f} kA는 평균보다 낮음 (평균 {current_mean:.2f} kA)")
    ...
    if max_proba_pct >= 90:
        st.write(f"   • 신뢰도: **{max_proba_pct:.1f}%** (높음 ✅)")
        st.write(f"   • 액션: 예측을 신뢰하고 해당 공정 파라미터로 운영 진행")
    elif max_proba_pct >= 70:
        st.write(f"   • 신뢰도: **{max_proba_pct:.1f}%** (중간 ⚠️)")
        st.write(f"   • 액션: 추가 모니터링 필요, 실시간 센서 데이터와 함께 검증")
    else:
        st.write(f"   • 신뢰도: **{max_proba_pct:.1f}%** (낮음 ❌)")
